...
*/
define('simple_draft_list_version','1.5');
...
function generate_draft_list_code($list_limit,$list_type,$list_order,$scheduled,$icon,$folder,$author_get,$source) {
    ...
    $drafts = $wpdb->get_results("SELECT post_title, post_status, display_name FROM $wpdb->posts A, $wpdb->users B WHERE B.ID = A.post_author AND (post_status = 'draft'".$status.") AND post_title NOT LIKE '!%'".$type." ORDER BY ".$order.$limit);
