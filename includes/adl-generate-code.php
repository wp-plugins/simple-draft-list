...
function adl_generate_code( $list_limit = '', $list_type = '', $list_order = '', $scheduled = '', $icon_folder = '', $date_format = '', $created = '', $modified = '', $cache_time = '', $template = '' ) {
    ...
    if ( ( $cache_time == 'no' ) or ( $cache_time == 'error' ) ) {
        $code = false;
    } else {
        $cache_key = 'adl_' . md5( $list_limit . $list_type . $list_order . $scheduled . $icon_folder . $date_format . $created . $modified . $cache_time . $template );
    ...
    if ( !$code ) {
        ...
        $error = false;
        if ( !is_numeric( $list_limit ) ) { $code .= adl_report_error( 'The limit is invalid - it must be a number', $plugin_name, false ); $error = true; }
        if ( ( $list_type != 'post' ) && ( $list_type != 'page' ) && ( $list_type != '' ) ) { $code .= adl_report_error( 'The list type is invalid - it must be blank, "post" or "page".', $plugin_name, false ); $error = true; }
        if ( ( $sort_field == '' ) or ( $sort_sequence == '' ) ) { $code .= adl_report_error( 'The order is invalid - please view the instructions for valid combinations', $plugin_name, false ); $error = true; }
        if ( ( $scheduled != 'no' ) && ( $scheduled != 'yes' ) ) { $code .= adl_report_error( 'The scheduled parameter must be either "Yes" or "No"', $plugin_name, false ); $error = true; }
        if ( ( $cache_time != 'no' ) && ( !is_numeric( $cache_time ) ) ) { $code .= adl_report_error( 'The cache time is invalid - it should either be a number of "No"', $plugin_name, false ); $error = true; }
        if ( strpos( $template, '%draft%' ) === false ) { $code .= adl_report_error( 'The template must include the %draft% tag', $plugin_name, false ); $error = true; }
        ...
        if ( ( $created == -1 ) or ( !$created ) ) { $code .= adl_report_error( 'The created parameter is invalid', $plugin_name, false ); $error = true; }
        $created = date( 'Y-m-d H:i:s', $created );

        if ( $modified != '' ) { $modified = '-' . $modified; } else { $modified = $far_past; }
        $modified = strtotime( $modified );
        if ( ( $modified == -1 ) or ( !$modified ) ) { $code .= adl_report_error( 'The modified parameter is invalid', $plugin_name, false ); $error = true; }
        ...
        if ( !$error ) {
            ...
            if ( $drafts ) {
                ...
                foreach ( $drafts as $draft_data ) {
                    ...
                    if ( ( ( current_user_can( 'edit_posts' ) ) && ( $post_type == 'post' ) ) or ( ( current_user_can( 'edit_pages' ) ) && ( $post_type == 'page' ) ) ) { $editor = true; } else { $editor = false; }

                    if ( ( $draft_title == '' ) && ( $editor ) ) { $draft_title = '[No Title]'; }
                    ...
                    if ( ( $date_accept ) && ( $draft_title != '' ) && ( strtolower( get_post_meta( $post_id, 'draft_hide', true ) ) != 'yes' ) && ( ( $list_limit == 0 ) or ( $valid_draft <= $list_limit ) ) ) {
                        ...
                        if ( $post_status == 'future' ) { $icon_url = '<img src="' . $icon_folder . 'scheduled.png" alt="Scheduled" title="Scheduled">'; } else { $icon_url = ''; }
