...
function adl_generate_code( $list_limit = '', $list_type = '', $list_order = '', $scheduled = '', $icon_folder = '', $date_format = '', $created = '', $modified = '', $cache_time = '', $template = '' ) {
	...
	if ( !$code ) {
		...
		$plugin_name = 'Artiss Draft List';
		$code = "\n<!-- " . $plugin_name . " v" . artiss_draft_list_version . " | http://www.artiss.co.uk/draft-list -->\n";
		...
		$error = false;
		if ( !is_numeric( $list_limit ) ) { $code .= adl_report_error( __( 'The limit is invalid - it must be a number', 'artiss-draft-list' ), $plugin_name, false ); $error = true; }
		if ( ( $sort_field == '' ) or ( $sort_sequence == '' ) ) { $code .= adl_report_error( __( 'The order is invalid - please view the instructions for valid combinations', 'artiss-draft-list' ), $plugin_name, false ); $error = true; }
		if ( ( $list_type != 'post' ) && ( $list_type != 'page' ) && ( $list_type != '' ) ) { $code .= adl_report_error( __( 'The list type is invalid - it must be blank, "post" or "page".', 'artiss-draft-list' ), $plugin_name, false ); $error = true; }
		if ( ( $scheduled != 'no' ) && ( $scheduled != 'yes' ) ) { $code .= adl_report_error( __( 'The scheduled parameter must be either "Yes" or "No"', 'artiss-draft-list' ), $plugin_name, false ); $error = true; }
		if ( ( $cache_time != 'no' ) && ( !is_numeric( $cache_time ) ) ) { $code .= adl_report_error( __( 'The cache time is invalid - it should either be a number of "No"', 'artiss-draft-list' ), $plugin_name, false ); $error = true; }
		if ( strpos( $template, '%draft%' ) === false ) { $code .= adl_report_error( __( 'The template must include the %draft% tag', 'artiss-draft-list' ), $plugin_name, false ); $error = true; }
		...
		if ( ( $created == -1 ) or ( !$created ) ) { $code .= adl_report_error( __( 'The created parameter is invalid', 'artiss-draft-list' ), $plugin_name, false ); $error = true; }
		$created = date( 'Y-m-d H:i:s', $created );

		if ( $modified != '' ) { $modified = '-' . $modified; } else { $modified = $far_past; }
		$modified = strtotime( $modified );
		if ( ( $modified == -1 ) or ( !$modified ) ) { $code .= adl_report_error( __( 'The modified parameter is invalid', 'artiss-draft-list' ), $plugin_name, false ); $error = true; }
		...
		if ( !$error ) {
			...
			if ( $icon_folder == '' ) {
				$icon_folder = WP_PLUGIN_URL . '/simple-draft-list/images/';
			} else {
			...
			echo "SELECT A.id, post_type, post_title, post_status, display_name, user_url, post_date, post_modified" . $sql_content . " FROM $wpdb->posts A, $wpdb->users B WHERE B.ID = A.post_author AND (post_status = 'draft'" . $status . ") AND post_title NOT LIKE '!%'" . $type . " ORDER BY " . $order;
			...
			if ( $drafts ) {
				...
				foreach ( $drafts as $draft_data ) {
					...
					if ( ( $draft_title == '' ) && ( $can_edit ) ) { $draft_title = '[No Title]'; }
					...
					if ( ( $date_accept ) && ( $draft_title != '' ) && ( strtolower( get_post_meta( $post_id, 'draft_hide', true ) ) != 'yes' ) && ( ( $list_limit == 0 ) or ( $valid_draft <= $list_limit ) ) ) {
						...
						$alt_title = __( 'Scheduled', 'artiss-draft-list' );
					   ...
					   $this_line = str_replace( '%author+link%', $author_link, $this_line );
						...
						if ( $draft_title != '' ) { $draft = $draft_title; } else { $draft = '(no title)'; }
