...
function adl_add_custom_box() {
	...
	$box_title = __( 'Draft List', 'artiss-draft-list' );
...
//add_action( 'add_meta_boxes', 'adl_add_custom_box' );
...
function adl_custom_box( $post ) {
	...
	echo '<label for="adl_hide">' . __( 'Hide from Draft List?', 'artiss-draft-list' ) . '&nbsp;</label> ';
...
function adl_save_postdata( $post_id ) {
    ...
    if ( isset( $_POST[ 'adl_hide' ] ) ) {
        update_post_meta( $post_id, 'draft_hide', $_POST[ 'adl_hide' ] );
    }

    // Whenever a post is saved, delete any cached draft lists
    global $wpdb;
