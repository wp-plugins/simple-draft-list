...
Where `template` is a default layout (see **Templates** section below) and `parameters` is an optional list of parameters, each separated by an ampersand (&)
...
No, I've paid particular attention to backwards compatibility - if you have set up your blog pre version 2 then the code you used should operate the same. However, you won't be benefiting from caching and various other improved features, so please update your code as soon as you can.
